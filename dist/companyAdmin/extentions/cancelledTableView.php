...
$result223 = $conn->query("SELECT a.id,a.companyID,a.terminalID,a.bustripID,a.customerID,a.origin,a.destination,a.duration,a.departure_time,a.number_of_seats,a.fare_amount,a.total_amount,a.payment_status,a.booked_at,a.booking_status,a.reference_id,b.trip_date,b.tripID,b.busID FROM booking_tbl a RIGHT OUTER JOIN bus_trip b ON(a.bustripID = b.id) WHERE a.companyID = '$id'  ");
...
<table id="bookTable" class="table table-stripeds table-borderless">
    ...
    <tbody>
        <?php foreach ($details as $detail) :
            if ($detail["booking_status"] == 'cancelled') { ?>
                <tr>
                    <td><?php echo $x++; ?></td>
                    <td class="">
                        <?php
                        ...
                        if ($detail['payment_status'] == 'paid') { ?>
                            <div class="dropdown px-0">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    <form method="post">
                                        <input type="hidden" name="booking_tbl_id" value="<?php echo $detail["id"]; ?>">
                                        <button class="dropdown-item" type="submit" name="updatepayment_refunded" value="<?php echo $detail['id']; ?>">Set to refunded</button>
                                    </form>
                                </div>
                            </div>
                        <?php } else if ($detail['payment_status'] == 'refunded') {
                            echo 'Refunded';
                        } else {
                            echo 'Not Paid';
                        }
...
<script>
    $(document).ready(function() {
        ...
        function fill_datatable(filter_termid = '', filter_route = '', filter_date = '') {
            var datatable = $('#cancelledbookTable').DataTable({
                ...
                "searching": false,
                "ajax": {
                    url: "fetchbookingcancelledtable.php",
                    type: "POST",
                    data: {
        ...
        $('#filter').click(function() {
            var filter_termid = $("#filter_termid_booking :selected").val();
            var filter_route = $("#filter_route_booking :selected").val();
            var filter_date = $("#filter_date_booking").val();

            if (filter_termid != '' && filter_route != '' && filter_date == '') {
                $('#cancelledbookTable').DataTable().destroy();
                ...
                $('#cancelledbookTable').DataTable().destroy();
                $('#cancelledbookTable').dataTable().fnClearTable();
            }
        });
    });
</script>