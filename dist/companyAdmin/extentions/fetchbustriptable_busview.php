...
$column = array('id', 'route', 'date', 'time', 'fare', 'driver', 'conductor', 'assigned by', 'action');
$query = "SELECT * FROM bus_trip WHERE companyID = $id ";

if (isset($_POST['filter_bus']) && $_POST['filter_bus'] != '' && isset($_POST['filter_date_range']) && $_POST['filter_date_range'] != '') {
    $str_arr = explode(" - ", $_POST['filter_date_range']);
    $begin = new DateTime($str_arr[0]);
    $end   = new DateTime($str_arr[1]);
    $operatesF = $begin->format("Y-m-d");
    $operatesE = $end->format("Y-m-d");
    $query .= 'AND trip_date BETWEEN "' . $operatesF . '" AND "' . $operatesE . '" AND busID = "' . $_POST['filter_bus'] . '" ';
}
// if(isset($_POST['filter_bus']) && $_POST['filter_bus'] != '')
// {
//     $query .= 'AND busID = '.$_POST['filter_bus'].' ';
// }
...
foreach ($result as $row) {
    ...
    <div class="dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
            <a href="bus_trip_view.php#editBusTripModal" class="dropdown-item" data-bs-toggle="modal" data-userid="' . $row['id'] . '">Change Bus</a>
            <a href="bus_trip_view.php#editBusTripModaldc" class="dropdown-item" data-bs-toggle="modal" data-userid="' . $row['id'] . '">Change Driver & Conductor</a>
